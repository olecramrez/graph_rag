...
def _build_command(
    ...
    cmd = [sys.executable, "-u", str(BASE_DIR / "index3.py")]
    ...
    if all_bases:
        ...
        cmd.extend(["--base", base_names[0] if base_names else ""])
...
def _start_indexing(
    ...
    if not all_bases and not base_names:
        st.error("Selecione pelo menos uma base para indexar.")
        return
    ...
    if mode.startswith("Rede"):
        ...
        env["RAG_DISABLE_FAISS_NETWORK_FALLBACK"] = "1"
...
st.set_page_config(
    layout="wide",
    page_title="Indexacao RAG - App3",
)

_init_state()
_refresh_process_state()

st.title("Indexacao RAG - App3")
...
with st.sidebar:
    ...
    if not all_bases:
        if not bases:
            ...
            default_selection = st.session_state.get("idx_selected_bases") or [default_base]
            default_selection = [base for base in default_selection if base in bases]
            selected_bases = st.multiselect(
                "Bases",
                bases,
                default=default_selection,
                key="idx_selected_bases",
                help="As bases selecionadas serao indexadas em sequencia, uma apos a outra.",
            )
    ...
    if all_bases:
        ...
        if selected_bases:
            st.caption(f"Bases selecionadas: {len(selected_bases)}")
            for selected_base in selected_bases:
                st.caption(f"`{get_documents_dir(selected_base)}`")
            base_folder = get_documents_dir(selected_bases[0])
        else:
            base_folder = DOCUMENTOS_ROOT
            st.caption("Nenhuma base selecionada.")
        if st.button(
            "Abrir primeira pasta selecionada",
            use_container_width=True,
            disabled=not bool(selected_bases),
        ):
    ...
    st.checkbox(
        ...
        help="Recria o FAISS/id_map do zero a partir dos chunks existentes.",
    )
