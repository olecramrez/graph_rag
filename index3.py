...
def run_index_for_base(
    ...
    # Auto-recuperacao: se chunks sumiram, reprocessa todos os PDFs da base.
    if not chunks_path.exists() and not to_process:
        all_pdfs = sorted(
            p.name for p in source_dir.glob("*.pdf")
            if p.is_file()
        )
        if all_pdfs:
            to_process = all_pdfs
            removed_files = []
            print(
                "[RECOVERY] chunks.jsonl ausente. "
                "Reprocessando todos os PDFs da base."
            )
...
def main():
    ...
    parser.add_argument(
        "--bases",
        nargs="+",
        help="Lista de bases para indexar em sequencia",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Use quando quiser reindexar todos os chunks ja existentes."
        ),
    )
    args = parser.parse_args()

    if args.all or args.bases:
        bases = get_available_bases() if args.all else args.bases

        if not bases:
            print("[WARN] Nenhuma base encontrada.")
            return

        print("\n===== INDEXACAO DE MULTIPLAS BASES =====")
